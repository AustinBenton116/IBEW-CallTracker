...
def run_scraper():
    ...
    if response.status_code == 200:
        ...
        for p in paragraphs:
            ...
            if is_header:
                if current_local: all_locals.append(current_local)
                
                # EXTRACT LINK: Look for the <a> tag
                link_tag = p.find('a', href=True)
                detected_link = link_tag['href'] if link_tag else None
            ...
            if not current_local: continue

            # ... (Wage, Benefit, and Status parsing logic remains identical to your previous version)
            ...
            if (is_wage_keyword or has_money_sign or text.startswith("$")) and not is_benefit and not contains_hidden_phone:
                ...
                for char in ["=", "-", "–", "—", ":", "$", "*"]: clean_scale = clean_scale.replace(char, "")
                ...
                if wage_numbers:
                    ...
                    if 15.0 < found_wage < 120.0 and len(text) < 85:
                        if found_wage not in current_local["wage_values_seen"]:
                            ...
                            if label == text or (any(char.isdigit() for char in label) and len(label) < 3): label = ""
                            display_val = f"${found_wage:.2f} ({label})".replace(" ()", "").strip()
                            current_local["wage_scales"].append(display_val)
                            if found_wage > current_local["highest_wage"]: current_local["highest_wage"] = found_wage
                        continue

            if is_benefit or "JOBLINE" in text_upper: continue
            if "JOB CALLS" in text_upper or "AVAILABLE FOR" in text_upper:
                if not current_local["status_info"]:
                    ...
                    if count_match: current_local["call_count"] = int(count_match.group(1))
                continue
            if text_upper.startswith("WORK ") or text_upper.startswith("LOCAL "):
                ...
                if len(first_sentence) < 75 and ":" not in first_sentence and not any(bad in first_sentence_upper for bad in ["MILLION", "BILLION", "PEORIA", "GALESBURG", "QUINCY"]):
                    if not current_local["status_info"]:
                        ...
                        if any(w in first_sentence_upper for w in ["GOOD", "STEADY", "PICKING UP", "BUSY"]): current_local["has_work"] = True

        if current_local: all_locals.append(current_local)
        for local in all_locals:
            if not local["status_info"]: local["status_info"] = "No explicit job details listed."
            if local["wage_scales"]:
                local["wage_scale_display"] = "\n".join([w.replace(" ()", "").strip() for w in local["wage_scales"]])
            else: local["wage_scale_display"] = "N/A"
            del local["wage_scales"]
            del local["wage_values_seen"]

        with open("jobs_data.json", "w") as f:
            json.dump({"locals": all_locals}, f, indent=4)
        print(f"Extraction successful! Records: {len(all_locals)}")
